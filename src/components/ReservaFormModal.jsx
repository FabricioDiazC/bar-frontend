  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex justify-center items-center p-4">
      <div className="bg-bar-card border border-zinc-800 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl relative">
        <button onClick={() => { resetForm(); onClose(); }} className="absolute top-4 right-4 text-zinc-400 hover:text-white cursor-pointer"><FaTimes size={20} /></button>
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="relative">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder="Escribe nombre..." value={busqueda} onChange={(e) => { setBusqueda(e.target.value); setClienteConfirmado(false); }}
                  className={`w-full bg-zinc-900 border ${clienteConfirmado ? 'border-green-900/50' : 'border-zinc-700'} rounded p-2 pl-10 text-bar-text focus:outline-none focus:border-bar-accent`} />
              </div>
            ...
            {/* Fila: Personas, Fecha y Hora Inicio */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select name="estado" value={formData.estado} onChange={handleChange} className="w-full bg-zinc-900 border border-zinc-700 rounded p-2 focus:outline-none focus:border-bar-accent">
            ...
            <div className="pt-4 border-t border-zinc-800 flex justify-end gap-3">
              <button type="button" onClick={() => { resetForm(); onClose(); }} className="px-4 py-2 text-bar-muted hover:text-white cursor-pointer">Cancelar</button>
              <button type="submit" className="bg-bar-accent hover:bg-yellow-600 text-black font-medium px-6 py-2 rounded cursor-pointer transition">Guardar Reserva</button>
            </div>
          </form>
        </div>
      </div>
