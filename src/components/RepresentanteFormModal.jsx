...
import { FaTimes, FaUserTie, FaPhoneAlt, FaUserTag } from 'react-icons/fa';
import { toast } from 'react-toastify';

export default function RepresentanteFormModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.post('representantes/', formData);
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                <FaUserTie className="absolute left-3 top-3.5 text-zinc-600" />
                <input type="text" name="nombre" required value={formData.nombre} onChange={handleChange} placeholder="Ej: Carlos Gómez"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" name="apodo" value={formData.apodo} onChange={handleChange} placeholder="Ej: Charly"
                  className="w-full bg-zinc-900 border border-zinc-700 rounded-xl p-3 pl-9 text-sm focus:border-bar-accent outline-none transition-all" />
              </div>
            </div>
